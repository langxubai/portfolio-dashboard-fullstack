...
## 开发阶段总结：Phase 7 - 核心算法迭代与数值精度强化

我们在最新迭代中修复了大量深层财务计算的精度、汇率核算以及时序对齐的 Bug，并夯实了底层算法的准确性：

### 1. 收益率曲线算法重构 (TWR) 与时区对齐
- **时间加权收益率 (TWR) 修复**：彻底摒弃了之前由于粗略累计净增比率在面临遭遇“抽资”、“本金空仓”时的数学畸变漏洞（剧烈掉落至 -100%）。现采用严格的 TWR 时间加权逻辑计算每一日期的复利斜率，中途入金出金均不再破坏对组合长久以来的表现评估。
- **跨时区 Index 融合修复**：修复了合成包含 Custom 在内的多源历史行情时，因为 `tz-naive`（Pandas 生成的历史行情）与 `tz-aware`（特定手工入库价格）的 DatetimeIndex 不兼容导致的 `"Cannot join tz-naive with tz-aware..."` 报错。在聚合前主动剥离清洗掉所有的 timezone 属性并以底层本地标准时间维为基准。

### 2. 基准货币与外汇换算 (Forex)
- **多币种等值汇总**：后端全面支持通过读取系统中选定的 Base Currency，并在市值合并时经过向 `yfinance` 挂档自动拉取并缓存实时的法币汇率（如 `CNYUSD=X` 或 `USDCNY=X` 估算价），最终使不同计价体系组合的资产（如美股 USD + A股 CNY）安全合并于一张以目标基准法定货币绘制的图表中。
- **原生开放 Forex 资产种类**：为系统原生支持了 `Forex` 类别以及对应的外汇价格获取映射。

### 3. 数据结构精度扩容与微量残币处理
- **全面 8 位小数精度**：所有下沉至 Pydantic 表现层和运算层的财务数字接口全链路扩容，完美支持了对 8 位小数点的精细存取，贴合了加密货币（Crypto）碎片化的真实存储要求。
- **清仓零头沉淀兜底**：设立了绝对值 `< 1e-8` 的残余持仓量清除阈值，防止高精度下由于小数无限相减引发的持有微弱“无名分份额”持续占用核心遍历计算内存。

### 4. Custom 价格记录并发污染修正
- **重名资产污染隔离**：修正了针对用户在不同账户下同时建立名称或代码冲突的 Custom 资产时，全局组合估算价格错误互相覆写混合投喂的 BUG。现在依靠底层 `asset_id` （UUID）做到彻底安全隔离查询；并在局部时间窗无更新时自动沿用前值的最新沉淀。
