...
## 开发阶段总结：Phase 7 - 前端体验深化与稳定性打磨

在近期的迭代中，我们对前端应用的细节体验和稳定性进行了深度的打磨与修复：

### 1. 首页看板呈现优化
- **看板视觉与图表重构**：将资产分配饼图以及未实现收益柱状图中的图例和坐标轴基础，由生硬的代码 (Symbol) 统一替换为完整的资产名称 (Name)，去除了图表中的冗余横坐标变量名，视觉效果干净清爽。
- **动态过滤沉淀资产**：在各资产未实现收益的柱状图中引入了智能过滤机制，自动隐藏当前持仓量已清零的“历史沉淀资产”，保持界面整洁。
- **收益率基准对齐**：将历史净值曲线的基准算法重构为当前所有持有资产的**基于成本加权的回报率**，彻底对齐了看板顶部的 Headline 核心数据。

### 2. 交互输入与交易防呆校验
- **精细化时间输入支持**：废弃了 Streamlit 默认的粗粒度 15 分钟间隔组件，替换为自由度极高的文本框，并支持无冒号盲填（四位/六位纯数字，如 `1514`、`151414`）自动解析规范化。
- **推导四舍五入防溢出**：修复了输入总金额反推单价/数量时可能带来的无限小数位导致入库失败的问题，目前强制截断保存 8 位小数 (`quantize`)。
- **交易列表人性化排序**：在录入流水时的资产和账户下拉列表中，自动基于名称/符号字典序进行排序，并智能记忆/置顶最新创建的项目，加快日常录入速度。

### 3. 底层通讯防断连与浮点兜底
- **解决静默掉线/500报错**：通过为下层 `api_client` 接入具备自动连接池维护和复用能力的持久化 `httpx.Client`，植入了带自动重试试错（Retrying）的网络防抖动机制，终结了由于 Streamlit Tab 静置过久导致请求假死必须 F5 刷新的体验痛点。
- **细小浮点残余抹平**：为了避免接近满额清盘造成的小数点后 8 位的 `1e-8` 残留，界面对近乎于 0 的数值自动进行抹平显示，采用 `Decimal.normalize()` 解除无意义的科学计数法零头显示。
